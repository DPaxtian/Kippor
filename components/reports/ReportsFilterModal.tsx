...
export function ReportsFilterModal({ visible, filters, onChange, onClose }: ReportsFilterModalProps) {
  ...
  function set(partial: Partial<ReportsFilters>) {
  ...
  function handleReset() {
    onChange({ period: 'month', customFrom: '', customTo: '', paymentMethods: [], labelIds: [] });
  }
              ...
              <View className="flex-row items-center justify-between px-5 pb-3">
                ...
                {activeCount > 0 && (
                  <Pressable onPress={handleReset} className="active:opacity-60">
                    <Text style={{ color }} className="text-sm font-semibold">{t('common.clear')}</Text>
                  </Pressable>
                )}
              </View>
